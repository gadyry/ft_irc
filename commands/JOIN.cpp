...
void Server::_cmdJoin(Client *client, std::vector<std::string> &tokens) {
  ...
  for (size_t i = 0; i < channels.size(); ++i) {
    ...
    if (channel->isMember(client)) {
      std::string ermsg = ERR_USERONCHANNEL(client->getNickname(),
    ...
    if (channel->getLimitUser() > 0 &&
        channel->getMemSize() >= channel->getLimitUser()) {
    ...
    /*theyre added so removing the invite in xase of leaving they wont have
    access to join agn they need a new invite token*/
    channel->removeInvite(client->getNickname());

    std::string jMsg = client->getNickname() + "!" + client->getUsername() +
                       "@" + client->getHost();
    ...
    if (channel->ch_getTopic().empty()) {
      ...
      std::string topicMsg =
          RPL_TOPIC(client->getNickname(), chName, channel->ch_getTopic());
